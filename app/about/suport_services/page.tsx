...
export default async function SupportServicesPage() {
  ...
  const hero = sections.find((s: any) => s.key === "support-hero") || {};
  const cards = sections.find((s: any) => s.key === "support-cards")?.items || [];
        ...
        <div className="w-full px-4 md:px-14">
          ...
          <div className="flex flex-col items-center text-center mb-12">
            ...
            <h1 className="text-[30px] md:text-[40px] lg:text-[48px] font-bold text-[#113d29] leading-tight mb-4">
              {hero.titleMain || "Support Services"}
            </h1>
            <p className="text-[16px] md:text-[18px] text-gray-600 max-w-3xl mx-auto leading-[1.6]">
              {hero.description || "We are committed to providing you with the best support. Find the resources and help you need for a successful experience at the Expo."}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {cards.length > 0 ? (
              cards.map((card: any, idx: number) => {
                const IconComponent = idx === 0 ? Headphones : idx === 1 ? Ticket : Megaphone;
                return (
                  <div key={idx} className="bg-[#f8f9fa] border border-gray-200 border-b-[4px] border-b-gray-300 rounded-xl p-8 hover:-translate-y-2 hover:shadow-xl hover:border-b-[#3b8c2a] transition-all duration-300 flex flex-col h-full group">
                    ...
                    <button className="text-[#e85d1c] font-bold text-[15px] md:text-[16px] tracking-wider uppercase hover:text-[#3b8c2a] transition-colors flex items-center gap-2">
                      {card.buttonLabel || "Read More"} <span>→</span>
                    </button>
                  </div>
                );
              })
