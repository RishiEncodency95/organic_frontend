        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h2 className="text-sm md:text-base font-semibold text-[#1b5e20] uppercase tracking-wider mb-0.5">
              {customProps?.title || "Industry Match Found"}
            </h2>
            <p className="text-xs md:text-sm font-medium text-gray-600 leading-snug">
              {customProps?.subtitle || "Your registered activities match Bharat Organic Expo."}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {items.length > 0 ? (
            items.map((item: any, idx: number) => {
              const IconComp = idx % 2 === 0 ? Leaf : Beaker;
              return (
                <div key={idx} className="border border-gray-100 rounded-xl p-3 md:p-4 relative overflow-hidden flex items-center justify-between shadow-sm">
                  <div className="flex flex-col z-10 w-[80%]">
                    <span className="text-[11px] md:text-xs font-semibold text-[#1b5e20] mb-0.5">NIC {item.code || "10795"}</span>
                    <h3 className="text-sm md:text-[15px] font-semibold text-gray-900 mb-1 leading-snug">{item.title}</h3>
                  ...
                  <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border border-gray-100 flex items-center justify-center shrink-0 bg-gray-50">
                    <IconComp size={28} className="text-[#1b5e20]" />
                  </div>
                </div>
              );
            })
