...
const GlobalPlatform = () => {
  const [data, setData] = useState(DEFAULT_GLOBAL);
    ...
    const fetchGlobal = async () => {
      try {
        ...
        if (serverData && isMounted) {
          ...
          let dynamicCards = DEFAULT_GLOBAL.cards;
          ...
          if (Array.isArray(rawCards) && rawCards.length > 0) {
            ...
            if (filtered.length > 0) {
              dynamicCards = filtered.map((c: any, idx: number) => {
                const fallback =
                  DEFAULT_GLOBAL.cards[idx % DEFAULT_GLOBAL.cards.length];
                return {
                  ...fallback,
          ...
          setData({
            ...
            badge:
              serverData.eyebrow || serverData.badge || DEFAULT_GLOBAL.badge,
            eyebrow:
              serverData.eyebrow || serverData.badge || DEFAULT_GLOBAL.eyebrow,
            titlePrimary:
              serverData.titlePrimary ||
              serverData.title?.line1 ||
              DEFAULT_GLOBAL.titlePrimary,
            titleSecondary:
              serverData.titleSecondary ||
              serverData.title?.highlight ||
              DEFAULT_GLOBAL.titleSecondary,
            description: serverData.description || DEFAULT_GLOBAL.description,
            listItems:
              ...
              serverData.keyPoint5
                ? [
                    serverData.keyPoint1 || DEFAULT_GLOBAL.listItems[0],
                    serverData.keyPoint2 || DEFAULT_GLOBAL.listItems[1],
                    serverData.keyPoint3 || DEFAULT_GLOBAL.listItems[2],
                    serverData.keyPoint4 || DEFAULT_GLOBAL.listItems[3],
                    serverData.keyPoint5 || DEFAULT_GLOBAL.listItems[4],
                  ]
                : Array.isArray(serverData.listItems) &&
                  serverData.listItems.length > 0
                ? serverData.listItems
                : DEFAULT_GLOBAL.listItems,
            cards: dynamicCards,
          });
        }
      } catch (err) {
        console.error("Failed to load GlobalPlatform data:", err);
      }
    };
      ...
      <div className="w-full relative z-10 flex flex-col xl:flex-row justify-between gap-4 lg:gap-10 xl:gap-4 items-center">
        ...
        <div className="w-full xl:w-[35%] flex flex-col items-center xl:items-start text-center xl:text-left relative z-20">
          ...
          <h2 className="text-[#1a3821] text-[22px] md:text-[26px] lg:text-[32px] leading-[1.2] md:leading-[1.2] font-semibold mb-4 font-poppins tracking-tight text-center xl:text-left">
            {data.titlePrimary}
            <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#f58634] to-[#f58634]/80">
              {data.titleSecondary}
            </span>
          </h2>
          ...
          <div className="flex flex-col gap-2 w-full sm:w-fit mx-auto xl:mx-0 px-2 sm:px-0">
            {data.listItems.map((item, index) => (
              <div
                key={index}
                className="flex items-start sm:items-center gap-3 group cursor-default text-left"
              >
                ...
                <span className="text-slate-700 font-semibold text-[13.5px] sm:text-[14px] md:text-[15px] lg:text-[16px] leading-snug transition-colors duration-300 group-hover:text-[#1a3821]">
                  {item}
                </span>
              </div>
            ))}
