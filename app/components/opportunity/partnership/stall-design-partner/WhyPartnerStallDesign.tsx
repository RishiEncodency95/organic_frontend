...
export default function WhyPartnerStallDesign() {
  ...
  const [categoryVal, setCategoryVal] = useState("Stall Design Partner");
  const [agreed, setAgreed] = useState(false);

  const data = WHY_PARTNER_STALL_DATA[0];
                ...
                {data.points.map((pt, idx) => (
                  <li key={idx} className="flex items-start gap-3">
                    <div className="w-6 h-6 rounded-full border border-[#1b5e20] bg-transparent flex items-center justify-center shrink-0 mt-0.5">
                      <Hammer className="w-3.5 h-3.5 text-[#1b5e20]" />
                    </div>
