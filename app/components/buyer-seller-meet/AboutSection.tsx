  ...
  return (
    <section className="pt-8 pb-8 bg-white font-inter relative overflow-hidden">
      ...
      <SectionContainer className="relative z-10">
        {ABOUT_DATA.map((data) => (
          <div key={data.id} className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
            ...
            >
              ...
              <div className="flex flex-col relative z-10">
                {data.links.map((link, i) => {
                  const commonProps = {
                    className: `flex items-center justify-between px-6 py-4 bg-transparent hover:bg-gray-50/80 transition-colors group ${i !== data.links.length - 1 ? "border-b border-gray-200" : ""
                      }`
                  };

                  const iconElem = typeof link.icon === "string" ? ICON_MAP[link.icon] : link.icon;
