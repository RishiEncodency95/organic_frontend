...
export default function DomesticBuyerForm() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const res = await buyerApi.getConfig();
  ...
  const submitFinal = async (pkg: any, transactionId: string, paymentStatus: string = "Completed") => {
    ...
    try {
      ...
      if (res && res.success !== false) {
        ...
        Swal.fire({ icon: 'error', title: 'Submission Failed', text: res?.message || 'Failed to submit registration.', scrollbarPadding: false });
      }
    } catch (err) {
      console.error("Error submitting buyer form:", err);
      Swal.fire({ icon: 'error', title: 'Error', text: 'An unexpected error occurred.', scrollbarPadding: false });
    } finally {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} onInvalid={(e) => (e.currentTarget as HTMLFormElement).classList.add('was-validated')} className="w-full space-y-2 animate-in fade-in duration-500">
      ...
      <div>
        <div className={sectionTitleWrapperClasses}><h3 className={sectionTitleClasses}> REGISTRATION CATEGORY 🔹</h3></div>
        <div className="space-y-4">
          <div className="flex items-center gap-2 px-2 mt-2">
            <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" />
            <p className="text-[11px] font-black text-slate-600 uppercase tracking-[0.2em] font-sans">
              {showMembershipOptions ? "Available Membership Packages" : "Available Registration Passes"}
            </p>
          </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <div className="relative z-10 flex flex-col items-center justify-center h-full px-4 pt-6 pb-6 gap-3">
                        ...
                        <button
                          type="button"
                          className="flex items-center gap-3 px-5 py-1 rounded-lg font-medium uppercase tracking-widest text-[11px] text-white w-fit pointer-events-none"
                          style={{
