...
import { CheckCircle2, ShieldCheck, User, Loader2, CheckCircle, X, ChevronsUpDown } from "lucide-react";
...
export default function GeneralForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    // Add real API logic here pointing to visitorApi.submitGeneral
    setTimeout(() => {
        setLoading(false);
        setSubmitted(true);
    }, 1500);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-gray-500 text-sm max-w-sm leading-relaxed">
          Thank you for registering as a General Visitor at Bharat Organic Expo. A confirmation will be sent to your email.
        </p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full space-y-2 animate-in fade-in duration-500">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-[#1a3352] uppercase tracking-wide font-poppins">General Visitor Registration</h3>
          <p className="text-[12px] text-gray-500 font-medium">Bharat Organic Expo 2027</p>
        </div>
      </div>

      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#ea580c]/20 pb-1.5 mb-4 gap-2 mt-8">
            <h3 className="text-[14px] font-bold text-[#ea580c] uppercase tracking-wide font-poppins">
                Personal Information
            </h3>
            <span className="text-[10px] font-bold bg-[#ea580c]/10 text-[#ea580c] px-2 py-1 rounded tracking-widest uppercase w-fit">Step 1 of 2</span>
        </div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2 h-[34px]">
              ...
              {!otpVerified.mobile && !otpSent.mobile && (
                <button type="button" onClick={() => handleSimulateOtp('mobile')} className={`bg-[#ea580c] text-white px-3 rounded-[2px] transition hover:bg-[#c2410c] h-full ${buttonTextClasses}`}>
                  {isVerifying.mobile ? <Loader2 className="animate-spin" size={14} /> : 'OTP'}
                </button>
              )}
              {otpSent.mobile && !otpVerified.mobile && (
                <button type="button" onClick={() => handleVerifyOtp('mobile')} className={`bg-[#ea580c] text-white px-3 rounded-[2px] transition hover:bg-[#c2410c] h-full ${buttonTextClasses}`}>
                  {isVerifying.mobile ? <Loader2 className="animate-spin" size={14} /> : 'Verify'}
                </button>
              )}
              {otpVerified.mobile && <CheckCircle size={18} className="text-[#ea580c] self-center shrink-0 ml-2" />}
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2 h-[34px]">
              ...
              {!otpVerified.email && !otpSent.email && (
                <button type="button" onClick={() => handleSimulateOtp('email')} className={`bg-[#ea580c] text-white px-3 rounded-[2px] transition hover:bg-[#c2410c] h-full ${buttonTextClasses}`}>
                  {isVerifying.email ? <Loader2 className="animate-spin" size={14} /> : 'OTP'}
                </button>
              )}
              {otpSent.email && !otpVerified.email && (
                <button type="button" onClick={() => handleVerifyOtp('email')} className={`bg-[#ea580c] text-white px-3 rounded-[2px] transition hover:bg-[#c2410c] h-full ${buttonTextClasses}`}>
                  {isVerifying.email ? <Loader2 className="animate-spin" size={14} /> : 'Verify'}
                </button>
              )}
              {otpVerified.email && <CheckCircle size={18} className="text-[#ea580c] self-center shrink-0 ml-2" />}
            </div>
          </div>
        </div>
      </div>
      ...
      <div>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-[#ea580c]/20 pb-1.5 mb-4 gap-2 mt-8">
            ...
            <span className="text-[10px] font-bold bg-[#ea580c]/10 text-[#ea580c] px-2 py-1 rounded tracking-widest uppercase w-fit">Step 2 of 2</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div>
          ...
          <div className="md:col-span-2 lg:col-span-4 mt-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4 bg-white p-5 border border-slate-200 rounded-sm shadow-sm">
                <label className="text-[11px] font-bold text-[#ea580c] uppercase tracking-wider block border-b border-slate-200 pb-2">Area of Interest <span className=" text-red-500">*</span></label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
