...
export default function ConferenceFeatureStrip() {
  ...
  return (
<<<<<<< HEAD
    <section className="container relative z-10 -mt-8 mx-auto max-w-[1400px] px-6 sm:-mt-10 rounded-2xl border border-gray-200/60 bg-[#fcfdfc] p-3 md:p-4 shadow-[rgba(0,0,0,0.02)_0px_1px_3px_0px,rgba(27,31,35,0.15)_0px_0px_0px_1px]">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 sm:gap-4">
        {strip.map((item, i) => (
          <Reveal key={item.label} delay={i * 70} direction={i % 2 === 0 ? "up" : "zoom"} className="h-full">
            <div className={`flex h-full items-center gap-3 transition-all duration-300 hover:-translate-y-1 ${i < strip.length - 1 ? "border-r border-gray-200/60" : ""}`}>
